...
class TestTypePreservation:
    def test_bounding_box_format_preserved(self) -> None:
        torch.manual_seed(0)
    ...
    def test_camera_intrinsics_image_size_preserved(self) -> None:
        torch.manual_seed(0)
        inputs, targets = _make_sample()
        inputs, _ = _apply_sequential(_standard_chain(), inputs, targets)
        # 3D scene transforms should not touch image_size.
        assert inputs["intrinsics"].image_size == (16, 24)


class TestPassthrough:
    def test_labels_unchanged_after_chain(self) -> None:
        torch.manual_seed(0)
        inputs, targets = _make_sample()
        original_labels = targets["labels"].clone()

        _, targets = _apply_sequential(_standard_chain(), inputs, targets)

        assert torch.equal(targets["labels"], original_labels)
    ...
    def test_nested_dict_structure_preserved(self) -> None:
        ...
        # Same keys, no extras introduced or dropped.
        ...
        assert set(targets.keys()) == {"boxes", "labels"}


class TestRandomStateConsistency:
    """Within one transform call, every dispatched kernel must see the
    same random parameters (the ``make_params`` contract)."""

    def test_flip_applies_same_axis_to_points_and_boxes(self) -> None:
        inputs, targets = _make_sample()
        points_before = inputs["points"].as_subclass(Tensor).clone()
        boxes_before = targets["boxes"].as_subclass(Tensor).clone()

        inputs, targets = RandomFlip3D(axis="x", p=1.0)(inputs, targets)

        points_after = inputs["points"].as_subclass(Tensor)
        boxes_after = targets["boxes"].as_subclass(Tensor)
        # Both point x and box center x must be negated by the same flip.
        assert torch.allclose(points_after[:, 0], -points_before[:, 0])
        assert torch.allclose(boxes_after[:, 0], -boxes_before[:, 0])

    def test_translate_applies_same_offset_to_points_and_boxes(self) -> None:
        torch.manual_seed(42)
        inputs, targets = _make_sample()
        points_before = inputs["points"].as_subclass(Tensor).clone()
        boxes_before = targets["boxes"].as_subclass(Tensor).clone()

        inputs, targets = RandomTranslate3D(translation_range=5.0, p=1.0)(
            inputs, targets
        )

        points_after = inputs["points"].as_subclass(Tensor)
        boxes_after = targets["boxes"].as_subclass(Tensor)
        # The xyz delta inferred from the points must match the delta
        # inferred from the box centers.
        box_delta = boxes_after[0, :3] - boxes_before[0, :3]
        point_delta = points_after[0, :3] - points_before[0, :3]
