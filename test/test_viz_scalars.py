...
class TestScalarCoercion:
    ...
    def test_multi_element_tensor_raises(self) -> None:
        ...
        # (a ValueError subclass) so RerunLogger can tell it apart from a
        # transport failure.
        with pytest.raises(LoggingInputError, match="must be a scalar but has 2"):
            _scalar_value("loss", torch.tensor([1.0, 2.0]))

    def test_bad_metric_leaves_the_recording_untouched(self, spy: _Spy) -> None:
        # Coercion runs up front, so neither the sound metric ahead of the bad
        # one nor the timeline cursor reaches Rerun.
        with pytest.raises(LoggingInputError, match="must be a scalar but has 2"):
            log_scalars({"loss": 1.0, "acc": torch.tensor([1.0, 2.0])}, step=0)
        assert spy.logged == []
        assert spy.times == []
