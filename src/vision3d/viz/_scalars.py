...
def log_scalars(
    ...
    *,
    ...
    Values are coerced before anything is logged, so a malformed metric raises
    :class:`LoggingInputError` without writing a partial step.
    ...
    """
    resolved = [
        (f"{prefix}/{name}" if prefix else name, _scalar_value(name, value))
    ...
    for entity, scalar in resolved:
        rr.log(entity, rr.Scalars(scalar), recording=recording)
