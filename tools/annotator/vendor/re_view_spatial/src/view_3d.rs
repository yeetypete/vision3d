...
use re_sdk_types::{Archetype as _, Component as _, ViewClassIdentifier, archetypes};
...
#[expect(dead_code)] // Forked identifier no longer routes through this.
type ViewType = re_sdk_types::blueprint::views::Spatial3DView;

impl ViewClass for SpatialView3D {
    fn identifier() -> ViewClassIdentifier {
        // Forked identifier so this can be registered alongside the built-in
        // 3D view rather than clashing with it.
        "Annotate3D".into()
    }
